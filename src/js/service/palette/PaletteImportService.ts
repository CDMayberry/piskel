...
module pskl {
    export module service {
        export module palette {
            const fileReaders = {
                'gpl': reader.PaletteGplReader,
                'pal': reader.PalettePalReader,
                'txt': reader.PaletteTxtReader,
                'img': reader.PaletteImageReader
            }
            ...
            export class PaletteImportService {
                ...
                private isImage_(file) {
                    return file.type.indexOf('image') === 0;
                };

                private getReader_(file, onSuccess, onError) {
                    var ReaderClass = this.getReaderClass_(file);
                    if (ReaderClass) {
                        return new ReaderClass(file, onSuccess, onError);
                    } else {
