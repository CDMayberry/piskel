    ...
    export module service {
        export module palette {
            export module reader {
                export abstract class AbstractPaletteFileReader {
                    file: any;
                    onSuccess: any;
                    onError: any;
                    colorLineRegexp: any;

                    constructor(file, onSuccess, onError, colorLineRegexp) {
                        this.file = file;
                        this.onSuccess = onSuccess;
                        this.onError = onError;
                        this.colorLineRegexp = colorLineRegexp;
                    ...
                    abstract extractColorFromLine(line);

                    read() {
                        pskl.utils.FileUtils.readFile(this.file, this.onFileLoaded_.bind(this));
                    }

                    onFileLoaded_(content) {
                        var text = pskl.utils.Base64.toText(content);
                        var lines = text.match(/[^\r\n]+/g);
