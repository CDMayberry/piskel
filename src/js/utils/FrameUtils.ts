module pskl {
     ...
     * @return {Image}
     */
            export function toImage(frame, zoom?, opacity?) {
                zoom = zoom || 1;
             ...
             */
            export function renderLine_(color, x, y, width, context) {
            ...
            export function mergeFrames_(frameA, frameB) {
             ...
             */
            export function addImageToFrame(frame, image, x, y) {
                var imageFrame = pskl.utils.FrameUtils.createFromImage(image);
                x = x - window.Math.floor(imageFrame.width / 2);
                y = y - window.Math.floor(imageFrame.height / 2);

                x = window.Math.max(0, x);
                y = window.Math.max(0, y);

                if (imageFrame.width <= frame.width) {
                    x = window.Math.min(x, frame.width - imageFrame.width);
                }

                if (imageFrame.height <= frame.height) {
                    y = window.Math.min(y, frame.height - imageFrame.height);
                }
            ...
            export function resize(frame, targetWidth, targetHeight, smoothing) {
            ...
            export function removeTransparency(frame) {
            ...
            export function createFromCanvas(canvas, x, y, w, h, preserveOpacity?) {
            ...
            export function createFromImageSrc(src, preserveOpacity, cb) {
                var image = new Image();
                image.addEventListener('load', function onImageLoaded() {
                    image.removeEventListener('load', onImageLoaded);
                    var frame = createFromImage(image, preserveOpacity);
                    cb(frame);
                });
                image.src = src;
            }
             ...
             */
            export function createFromImage(image, preserveOpacity?) {
                var w = image.width;
                var h = image.height;
            ...
            export function createFromImageData_(imageData, width, height, preserveOpacity) {
             ...
             * @return {Array<Frame>}
             */
            export function createFramesFromSpritesheet(image, frameCount) {
                var layout = [];
                for (var i = 0; i < frameCount; i++) {
                    layout.push([i]);
                }
                var chunkFrames = pskl.utils.FrameUtils.createFramesFromChunk(image, layout);
                return chunkFrames.map(chunkFrame => chunkFrame.frame);
            }
             ...
             */
            export function createFramesFromChunk(image, layout) {
            ...
            export function toFrameGrid(normalGrid) {
                var frameGrid = [];
