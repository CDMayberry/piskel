...
module pskl {
    export module UserSettings {
        ...
        export function get(key) {
            checkKeyValidity_(key);
            if (!(key in cache_)) {
                var storedValue = readFromLocalStorage_(key);
                if (typeof storedValue !== 'undefined' && storedValue !== null) {
                    cache_[key] = storedValue;
                } else {
                    cache_[key] = readFromDefaults_(key);
                }
            }
            return cache_[key];
        }
        ...
        export function set(key, value) {
            checkKeyValidity_(key);
            cache_[key] = value;
            writeToLocalStorage_(key, value);
        ...
        export function readFromDefaults_(key) {
            return KEY_TO_DEFAULT_VALUE_MAP_[key];
        }
        ...
        export function checkKeyValidity_(key) {
            //@ts-ignore, TODO: Remove after service conversion
            if (key.indexOf(pskl.service.keyboard.Shortcut.USER_SETTINGS_PREFIX) === 0) {
                return true;
            }

            var isValidKey = key in KEY_TO_DEFAULT_VALUE_MAP_;
            if (!isValidKey) {
